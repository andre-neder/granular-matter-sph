...
\begin{document}
    ...
    A typical implementation for a kernel function is the cubic spline kernel \cite{doi:10.1146/annurev.aa.30.090192.002551}. I this work a spiky kernel \cite{seblague} is used as it provided better results for granular matter.
    ...
    Implicit Incompressible Smoothed Particle Hydrodynamics (IISPH) is a variant of the SPH method that also addresses the challenge of maintaining incompressibility in fluid simulations. Unlike traditional SPH methods, IISPH employs an implicit formulation for the pressure term in the Navier-Stokes equations, allowing for larger time steps and improved stability.\cite{6570475} %This method is particularly useful in simulating incompressible fluid flows where maintaining constant density is essential, providing a more efficient and stable approach compared to explicit methods in certain scenarios, such as large-scale simulations or real-time applications.%
