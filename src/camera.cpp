...
Camera::Camera(Type type, gpu::Window* window, uint32_t width, uint32_t height, glm::vec3 eye, glm::vec3 center)
{
...
void Camera::update(double dt){
    ...
    _radius -= (float) (_window->getScrollOffset().y * 0.1);
    _radius = glm::max((float)_radius, 0.000001f);

    if (_window->getButtonState(GLFW_MOUSE_BUTTON_LEFT)) {
        _newPos = _window->getCursorPosition();
    ...
    _oldPos = glm::vec2(_window->getCursorPosition());

    if (_window->getKeyState(GLFW_KEY_C) && _timeout <= 0.00001){
        if(_type == eTrackBall){
            _type = eFirstPerson;
            _forward = glm::normalize(_center - _eye);
            _forward.y = 0.0f;
    ...
    if(_type == eTrackBall){
        ...
        if (_window->getKeyState(GLFW_KEY_W))
            _eye += forward * (float) delta_time * 3.f;
        if (_window->getKeyState(GLFW_KEY_A)) 
            _eye += left * (float) delta_time * 3.f;
        if (_window->getKeyState(GLFW_KEY_S))
            _eye += backwards * (float) delta_time * 3.f;
	    if (_window->getKeyState(GLFW_KEY_D))
            _eye += right * (float) delta_time * 3.f;
        if (_window->getKeyState(GLFW_KEY_LEFT_SHIFT))
            _eye += down * (float) delta_time * 3.f;
        if (_window->getKeyState(GLFW_KEY_SPACE))
...
glm::mat4 gpu::Camera::getProj()
{
    return glm::perspective(35.f, _width / (float)_height, 0.001f, 1000.f);
}

CameraData gpu::Camera::getCameraData()
{
    return {
        getView(),
        getProj()
